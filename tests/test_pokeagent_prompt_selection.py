...
from agents.prompts.paths import POKEAGENT_PROMPT_PATH, POKEAGENT_SYSTEM_PROMPT_PATH
...
def test_auto_system_uses_system_prompt_with_optimization_and_run_manager():
    load_mock = MagicMock(return_value="SYS_BODY")
    fake_opt = MagicMock()
    with patch.object(_POKE_MODULE, "MCPToolAdapter"), patch.object(_POKE_MODULE, "VLM"), patch.object(
        _POKE_MODULE, "create_prompt_optimizer"
    ...
    load_mock.assert_called_once()
    assert _filename_arg(load_mock) == POKEAGENT_SYSTEM_PROMPT_PATH
