# Pokemon Red Agent — Fixed constraints and callable tools
...
- **Note:** Gen 1 Game Boy has no L/R shoulder buttons.
...
These tools run as **local subagents** inside PokeAgent. One-step subagents (`subagent_reflect`, `subagent_verify`, `red_puzzle_agent`, `subagent_summarize`) each consume a **separate VLM call / global step** and inspect your **current screenshot** plus the **last N logged trajectories** (default 10 for reflect/verify, cap 50).
...
**`red_puzzle_agent`**
- **Required:** `location_name` (string) — puzzle location identifier from current game state (e.g. `RocketHideoutB2f`, `RocketHideoutB3f`)
- Use for spinner mazes, warp mazes, and other Gen 1 puzzle areas.
...
- *(no parameters)* — returns milestones, location, objective status, **completed objectives history**, **memory tree overview**, and run directory. Use **`process_memory`** when you only need memory details.
