...
data['location']     # 'PALLET TOWN'
data['grid']         # list of strings, e.g. ['##.I.##', '##...##'] - grid[y][x]
data['dimensions']   # {'width': int, 'height': int}
data['warps']        # [{'x': 6, 'y': 13, 'dest_map': 'PALLET_TOWN'}, ...]
data['objects']      # [{'x': 5, 'y': 2, ...}, ...]
data['connections']  # [{'direction': 'north', 'map': 'ROUTE_1'}, ...]
data['party']        # [{'species': 'Charmander', 'level': 5, 'hp': 20, 'max_hp': 20, 'moves': [...]}, ...]
data['grid_legend']  # 'I=player .=walkable #=wall ~=grass W=water D=door N=NPC ...'
```

The grid is the same ASCII map from your game state text with `I` at the player position and `N` marking NPC positions. `grid[y][x]` gives the tile. Walkable: `.`, `~`, `D`, `I`. Blocked: `#`, `N` (NPC), `W` (water), `t` (cuttable tree).
