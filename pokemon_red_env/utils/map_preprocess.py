...
from map_overrides import MAP_GRID_OVERRIDES, MAP_ELEVATION_OVERRIDES  # noqa: E402
...
_GRID_SYMBOL_TO_BEHAVIOR: Dict[str, "RedMetatileBehavior"] = {
    ...
    "G":  RedMetatileBehavior.IMPASSABLE,       # Card Key gate (blocked until opened)
    # Note: "O" (Poké Ball) is an overlay-only symbol — never placed in the static grid.
}


# Mapping from ASM map constants to processed_map names (Silph Co. only)
_CARD_KEY_MAP_CONST_TO_NAME: Dict[str, str] = {
    "SILPH_CO_2F":  "SilphCo2F",
    "SILPH_CO_3F":  "SilphCo3F",
    "SILPH_CO_4F":  "SilphCo4F",
    "SILPH_CO_5F":  "SilphCo5F",
    "SILPH_CO_6F":  "SilphCo6F",
    "SILPH_CO_7F":  "SilphCo7F",
    "SILPH_CO_8F":  "SilphCo8F",
    "SILPH_CO_9F":  "SilphCo9F",
    "SILPH_CO_10F": "SilphCo10F",
...
def parse_card_key_coords_asm(path: str) -> Dict[str, List[Tuple[int, int]]]:
    ...
    with open(path, "r", encoding="utf-8") as f:
        for line in f:
            line = line.strip()
            if not line or line.startswith(";") or line.endswith(":") or line == "db -1 ; end":
                continue
            # Format: db MAP_CONST, $Y, $X, gate_id
            m = re.match(r"db\s+(SILPH_CO_\w+)\s*,\s*\$([0-9A-Fa-f]+)\s*,\s*\$([0-9A-Fa-f]+)\s*,\s*\d+", line)
            if m:
                map_const = m.group(1)
                y = int(m.group(2), 16)
                x = int(m.group(3), 16)
                map_name = _CARD_KEY_MAP_CONST_TO_NAME.get(map_const)
                if map_name:
                    result[map_name].append((x, y))
    return dict(result)
...
def main():
    ...
    card_key_coords_path = os.path.join(root_dir, "data", "events", "card_key_coords.asm")
    card_key_dict = parse_card_key_coords_asm(card_key_coords_path)
    ...
    for header_file in header_files:
        ...
        # Overlay Card Key gate symbols for Silph Co. maps.
        if map_name in card_key_dict:
            for gx, gy in card_key_dict[map_name]:
                if 0 <= gy < coll_map_h and 0 <= gx < coll_map_w:
                    grid[gy][gx] = 'G'
                    behavior_map[gy][gx] = RedMetatileBehavior.IMPASSABLE
            print(f"[{map_name}] Applied {len(card_key_dict[map_name])} Card Key gate(s).")

        # Apply manual grid override (MAP_GRID_OVERRIDES), if present.
        # The override is applied AFTER all automated passes so it is the
        # final authority on grid symbols and metatile behaviors.
        if map_name in MAP_GRID_OVERRIDES:
            override_rows = MAP_GRID_OVERRIDES[map_name]
            if len(override_rows) != coll_map_h:
                print(
                    f"[{map_name}] WARNING: MAP_GRID_OVERRIDES height "
                ...
                for cy, override_row in enumerate(override_rows):
                    if len(override_row) != coll_map_w:
                        print(
                            f"[{map_name}] WARNING: MAP_GRID_OVERRIDES row {cy} width "
                ...
                print(f"[{map_name}] Applied MAP_GRID_OVERRIDES grid.")
        ...
        for cy in range(coll_map_h):
            ...
            for cx in range(coll_map_w):
                ...
                raw_row.append([t, int(beh), col, 0])   # elevation defaults to 0
            raw_tile_map.append(raw_row)

        # Apply elevation overrides (single-digit strings, same dims as grid).
        if map_name in MAP_ELEVATION_OVERRIDES:
            elev_rows = MAP_ELEVATION_OVERRIDES[map_name]
            if len(elev_rows) != coll_map_h:
                print(
                    f"[{map_name}] WARNING: MAP_ELEVATION_OVERRIDES height "
                    f"{len(elev_rows)} != map height {coll_map_h}; skipping."
                )
            else:
                for cy, elev_row in enumerate(elev_rows):
                    if len(elev_row) != coll_map_w:
                        print(
                            f"[{map_name}] WARNING: MAP_ELEVATION_OVERRIDES row {cy} width "
                            f"{len(elev_row)} != map width {coll_map_w}; skipping row."
                        )
                        continue
                    for cx, ch in enumerate(elev_row):
                        raw_tile_map[cy][cx][3] = int(ch)
                print(f"[{map_name}] Applied MAP_ELEVATION_OVERRIDES.")
